...
| `AddTxToFilter` | Adds an transaction to filter. |
| `AddTxoToFilter` | Adds an output to filter. |
| `EndBuildFilterLoad` | Finishes building a filter. |
| `BuildFilterAddFromTx` | Builds a `filteradd` message from a transaction. |
| `BuildFilterAddFromTxo` | Builds a `filteradd` message from a output. |
...
| `Probe` | Tells the scenario to record the incoming messages from the peer. |
