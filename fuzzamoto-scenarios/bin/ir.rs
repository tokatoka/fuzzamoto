...
use fuzzamoto::{
    ...
    targets::{
        BitcoinCoreTarget, ConnectableTarget, HasBlockChainRPC, HasGetBestBlockHash, Target,
    },
};
...
where
    TX: Transport,
    T: Target<TX> + HasGetBestBlockHash + ConnectableTarget + HasGetBestBlockHash,
{
...
pub fn probe_recent_block_hashes<T: HasBlockChainRPC>(
    target: &T,
    ...
    for (height, hash) in &hashes {
        let header_var = if let Some((header, _, _)) = meta.block_variables(&hash)
...
where
    TX: Transport,
    T: Target<TX> + ConnectableTarget + HasBlockChainRPC,
{
